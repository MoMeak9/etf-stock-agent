...
积极参与，解决提出的任何具体担忧，反驳他们逻辑中的弱点，并断言承担风险的好处以超越市场常规。专注于辩论和说服，而不仅仅是呈现数据。挑战每个反驳点，强调为什么高风险方法是最优的。请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。"""
        ...
        logger.info("Aggressive Analyst invoking LLM...")
        llm_start_time = time.time()

        response = llm.invoke(prompt)

        llm_elapsed = time.time() - llm_start_time
        logger.info(f"Aggressive Analyst LLM call completed in {llm_elapsed:.2f}s")

        argument = f"Aggressive Analyst: {response.content}"

        new_count = risk_debate_state["count"] + 1
        logger.info(f"Aggressive risk analyst completed, count: {risk_debate_state['count']} -> {new_count}")
